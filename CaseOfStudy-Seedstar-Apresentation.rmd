---
title: "Case of Study for Seedstar Internship"
...
## Introduction 
- This is a presentation of a case of study given as an assignment for the Seedstar Data Scientist Internship Position.
- To implement the case of study, it was used R programming language as main tool for data analysis. 
...
- It is perceveid that the median of the loan amount of the males with label equal '0' is almost 150
```{r}
...
## Exploration of the data
- After some process on the data, it is good to explore more by subsetting the data to get more insights.  
...
ggplot(males_label_1, aes(x = label, y = Amount)) + geom_boxplot() + ggtitle("Boxplot of the Amount per Label equal '1' by Males") + scale_y_continuous(limits = c(0,50)) 

ggplot(males_label_0, aes(x = label, y = Amount)) + geom_boxplot() + ggtitle("Boxplot of the Amount per Label equal '0' by Males") + scale_y_continuous(limits = c(0,100)) 
...
```{r}
ggplot(males_label_1, aes(x = Education_Status)) +  stat_count(width = 0.5) + xlab("Education Status") + ylab("Total of Males with Label 1") + labs(fill="label")  + ggtitle("Barplot of the Total Male with Label equals '1' based on Education Status") + facet_wrap(~Reason)
```
##
...
ggplot(aes(x = Age, y = Amount, fill = factor(label)), data = dataset ) +
  geom_point() + labs(fill="label") + ylim(c(0, 300))
```
